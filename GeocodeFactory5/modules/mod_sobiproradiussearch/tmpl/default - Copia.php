...
<form action="index.php" method="post" id="spSearchForm" >
	...
	<?php
		if ($tpl){
			...
			//echo $kwshow ;
			echo "<div class=\"spsearch_label\">Cosa vuoi fare?</div><input type=\"text\" name=\"sp_search_for\" value=\"\" class=\"form-control search-query\" autocomplete=\"off\" id=\"SPSearchBox2\" placeholder=\"\">";
			echo $keyw ;
			echo $rad . $lmb ; 
			echo $btn ;
		}
...
jQuery( document ).ready(function() {
	if(jQuery('#SPSearchBox').length) {
		if(jQuery('#SPSearchBox').val()!='...') jQuery('#SPSearchBox2').val(jQuery('#SPSearchBox').val());
	}	
	if(jQuery('#mj_rs_center_selector').length) {
		if(jQuery('#mj_rs_ref_lat').length)
			if(jQuery('#mj_rs_ref_lat').val()!='') jQuery('#mj_rs_mod_center_selector').val(jQuery('#mj_rs_center_selector').val());
	...
	jQuery('#spSearchForm').submit(function() {
		if (jQuery.trim(jQuery("#SPSearchBox2").val()) === "" && jQuery.trim(jQuery("#mj_rs_mod_center_selector").val()) === "") {
			if(jQuery("#msg-empty-form").length) return false;
			else {
				jQuery( "#spSearchForm" ).append( "<div id=\"msg-empty-form\">Ops! Sembra che tu non abbia compilato i campi di ricerca. Per cercare tra le offerte di lavoro, scrivi cosa vuoi fare e/o dove.</div>" );
				return false;
			}
