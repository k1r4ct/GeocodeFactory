...
class plggeocodefactoryPlg_geofactory_gw_jc30 extends GeofactoryPluginHelper{
	...
	public function getItemCoordinates($type, $ids, &$vCoord, $params){
		...
		$db->setQuery($query);
		$res = $db->loadObjectList();
		if ($db->getError()) { trigger_error("getItemCoordinates : DB reports: ".$db->stderr(), E_USER_WARNING);}
	...
	public function getItemPostalAddress($type, $id, $vAssign){
		...
		if 			(isset($vAssign["field_latitude"]) AND $vAssign["field_latitude"]==-2){
			...
			$res = $db->loadObjectList();
			if ($db->getError()) { trigger_error("getItemPostalAddress : DB reports: ".$db->stderr(), E_USER_WARNING);}
	...
	public function getAllSubCats($type, &$vCats, &$idTopCat){
		...
		if ($db->getError()) {	trigger_error("getCategories: DB reports: ".$db->stderr(), E_USER_WARNING);}
	}
	...
	public function markerTemplateAndPlaceholder(&$objMarker, $params){
		...
		foreach ($this->arBubbleFields as $fName){
			...
			if(strpos($dispo, 'field') !== false){
				$db = JFactory::getDBO();	
				$id_field = array_pop(explode('_', $dispo));
			
				$query = $db->getQuery(true);
				$query->select('value');
				$query->from($db->quoteName('#__fields_values'));
				$query->where("field_id = " . (int) $id_field . " AND item_id=" . (int) $article->id);			
				$db->setQuery($query, 0, 1);	
				$res = $db->loadObjectList();
				$objMarker->replace[$dispo] 	=  JHtml::_('content.prepare', $res[0]->value);
				continue ;
			}			
	...
	public function _getItemFromArticle($id, $field, $purejoomla=false){
		...
		if(strpos($field, 'field') !== false) {
			$id_field = array_pop(explode('_', $field));
			
			$query = $db->getQuery(true);
			$query->select('j.value as '. $field);
			$query->from($db->quoteName($this->plgTable).' AS a');
			$query->join('LEFT', '#__fields_values AS j ON j.item_id=a.id_content');
			$query->where("j.field_id = " . (int) $id_field . " AND a.id=" . (int) $id);			
			$db->setQuery($query, 0, 1);
